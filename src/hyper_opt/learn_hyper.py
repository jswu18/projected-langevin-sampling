...
from hyper_opt.make_sample import train_x, train_y, train_x_all
from hyper_opt.utils import custom_dataloader, check_convergence
...
class KernelsHyperOpt():
    ...
    def __init__(self, cost:Costs, kernel:gpytorch.kernels.Kernel, device, standardize =True, reg_para=0.001) -> None:
        self.cost = cost
        self.device = device
        self.alpha = None # initialised in data_loader, before we dont know length of alpha
        self.beta = None
    ...
    def _init_optimization(self,X,Y,lr1,lr2, n_hyper, batch_size):
        ...
        self.alpha = torch.nn.Parameter(torch.ones(self.X1.shape[0],device=self.device))
        ...
        trainable_parameters = [param for param in all_parameters if param.requires_grad]
    ...
    def _update_alpha(self,max_epochs=10000):
        ...
        for epoch in range(max_epochs):
            ...
            for X_batch, Y_batch in self.dl1:
                ...
                grad = 1/N_batch * torch.matmul(k_Xb_X1.T , self.cost.derivative(Y_batch,Y_batch_hat)) + self.reg_para * self.alpha
                ...
                with torch.no_grad():
                    cost_epoch += 1/N_batch * self.cost(Y_batch,Y_batch_hat)+ self.reg_para* torch.sum(self.alpha**2)

            cost_list.append(cost_epoch/len(self.dl1))
            print(f'Epoch {epoch+1}, Average Costs Alpha Optimisation: {cost_list[-1]:.2f}')

            if epoch > self.patience:
                if check_convergence(cost_list,patience=self.patience,min_delta=self.min_delta):
                    break
    ...
    def _update_beta(self,max_epochs=10000):
        ...
        for epoch in range(max_epochs):
            ...
            with torch.no_grad():
                ...
                print(f'Epoch {epoch+1}, Average Costs Beta Optimisation: {cost_list[-1]:.2f}')
    ...
    def _update_kernel_params(self,max_epochs=100):
        ...
        for epoch in range(max_epochs):
            ...
            print(f'Epoch {epoch+1}, Average Costs Hyerparameter Optimisation: {cost_list[-1]:.2f}')
    ...
    def fit(self, X:np.ndarray, Y:np.ndarray, batch_size, max_iter=100,lr1=1e-4,lr2=1e-4, n_hyper=0.2, patience=5, min_delta =0.001 ):
        
        #Initialise Dataloaders, optimisers, etc
        self._init_optimization(X,Y,lr1=lr1,lr2=lr2, n_hyper=n_hyper, batch_size=batch_size)
        ...
        counter = 0
        k_XX = self.kernel(self.X1,self.X1) + self.reg_para * torch.eye(self.X1.shape[0])
        ...
        if counter == max_iter:
            warnings.warn('The Algorithm has stopped due to reaching the maximum number of iterations. ', category=UserWarning)
