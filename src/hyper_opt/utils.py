...
class CustomDataset(Dataset):
    def __init__(self, X, Y):
        self.X = torch.tensor(X, dtype=torch.float32)  # Assuming X and Y are numpy arrays or lists
...
def check_convergence(cost_list,patience,min_delta):
    ...
    if all(abs(cost_list[-i] - cost_list[-i-1]) < min_delta for i in range(1, patience+1)):
